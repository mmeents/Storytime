...
namespace Storytime.Core.Agents {
  ...
  public class DirectorAgent : IDirectorAgent {
    ...
    public DirectorAgent(ILocalBaseAgentFactory baseAgentFactory, IMediator mediator, ILogger<DirectorAgent> logger) {
    ...
    public async Task DirectScene(int storyId, int sceneId, CancellationToken ct = default) {
      ...
      if (scene.Root == null) { 
        _logger.LogError("No scene returned in DirectScene for sceneId {sceneId}", sceneId);
      ...
      if (story.Root == null) {
        _logger.LogError("No story returned in DirectScene for storyId {storyId}", storyId);
      ...
      try { 
        ...
        _logger.LogError(ex, "Error running story {storyId} for DirectorAgent", storyId);
        throw new Exception($"Failed to run story {storyId} for DirectorAgent: {ex.Message}");
      }
    }
  }
}
