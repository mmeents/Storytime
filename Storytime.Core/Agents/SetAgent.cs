...
namespace Storytime.Core.Agents {
  ...
  public class SetAgent : ISetAgent {
    ...
    private readonly ILogger<SetAgent> _logger;

    public SetAgent(ILocalBaseAgentFactory baseAgentFactory, IMediator mediator, ILogger<SetAgent> logger) {
    ...
    public async Task PerformScene(int callSheetId, int storyId, CancellationToken ct = default) {
      ...
      if (callSheet.Root == null) {
        _logger.LogError("CallSheet with id {callSheetId} not found.", callSheetId);
      ...
      if (string.IsNullOrWhiteSpace(callSheet.Root.Data) || callSheet.Root.Data == "{}") {
        _logger.LogError("CallSheet {callSheetId} has no Script. Run the Director first.", callSheetId);
      ...
      try {      
        ...
        _logger.LogError(ex, "Error during performance of CallSheet {callSheetId}.", callSheetId);
        throw new Exception($"Error during performance of CallSheet {callSheetId}: {ex.Message}");
      }
    }
  }
}
